...
export default function BlogThoughts() {
  ...
  // ── Baca judul & konten sesuai bahasa aktif ───────────────────────────────
  const getTitle   = (post) => language === 'id' && post.title_id   ? post.title_id   : post.title
  const getContent = (post) => language === 'id' && post.content_id ? post.content_id : post.content
  ...
  const getReadTime = (post) => {
    const mins = Math.ceil(getWordCount(getContent(post)) / 200)
    return language === 'id' ? `${mins} menit baca` : `${mins} min read`
  }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filteredPosts.map(post => (
          <div key={post.id} onClick={() => setSelectedPost(post)}
            ...
            {/* Preview konten dalam bahasa aktif */}
            <p className="text-sm mb-3 line-clamp-3">
              {getContent(post).replace(/#{1,3} /g, '').replace(/\*\*/g, '').substring(0, 150)}...
            </p>
            ...
            <div className="flex items-center justify-between text-xs">
              ...
              <button className="flex items-center gap-1 text-accent-teal hover:underline">
                {b.readMore} <ChevronRight size={14} />
              </button>
            </div>
          </div>
        ))}
        ...
        <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className={`${bgPrimary} border-2 border-accent-teal rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto`}>
            ...
            <div className={`sticky top-0 ${bgSecondary} border-b ${borderClass} p-4 flex items-center justify-between z-10`}>
              ...
              <button onClick={() => setSelectedPost(null)} className="hover:text-white transition-colors ml-2 flex-shrink-0">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="p-6">
              ...
              <div className="flex items-center gap-4 mb-6 text-sm">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span>{getWordCount(getContent(selectedPost))} {b.words}</span>
                </div>
              </div>

              {/* Konten dalam bahasa aktif */}
