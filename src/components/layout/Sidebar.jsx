        ...
        <div className="p-4">
          ...
          <div className="mb-2">
            ...
            {openFolders['personal-info'] && (
              <div className="ml-6 mt-2 space-y-2">
                ...
                {tabItem('skills', <Zap size={14} className="text-accent-teal" />, a.skillsNav)}
                {tabItem('code-samples', <Code2 size={14} className="text-accent-blue" />, a.codeSamples)}
                ...
                {tabItem('particles', <Sparkles size={14} className="text-accent-teal" />, 'particles')}
          ...
          <div className="mt-6">
            <div className="mb-2 flex items-center gap-2">
              <ChevronDown size={16} />
              <span className="text-sm">{a.contacts}</span>
            </div>
        ...
        <div className="p-4">
          <div className="flex items-center justify-between mb-4">
            ...
            {selectedFilters.length > 0 && (
              <button onClick={clearAllFilters} className="text-xs text-accent-orange hover:text-accent-orange/80 transition-colors">
                {p.clearAll}
              </button>
            )}
          </div>
          ...
          <div className="space-y-2">
            {techFilters.map(filter => (
              <label key={filter.name} className={`flex items-center gap-2 cursor-pointer transition-colors p-2 rounded ${
                selectedFilters.includes(filter.name)
                  ? theme === 'dark' ? 'bg-accent-teal/10 text-white' : 'bg-accent-blue/10 text-gray-900'
                  : 'hover:text-white'
              }`}>
                <input type="checkbox" checked={selectedFilters.includes(filter.name)} onChange={() => toggleFilter(filter.name)}
                  className={`w-4 h-4 rounded cursor-pointer ${theme === 'dark' ? 'accent-accent-teal' : 'accent-accent-blue'}`} />
                <span className="text-xs flex-1">{filter.icon} {filter.name}</span>
                {selectedFilters.includes(filter.name) && <span className="text-xs text-accent-teal">✓</span>}
              </label>
            ))}
          </div>

          <div className="mt-4 pt-4 border-t border-dark-border">
            <p className="text-xs text-accent-blue mb-2">{p.filterTips}</p>
            <p className="text-xs opacity-60">{p.filterHint}</p>
          </div>
        </div>
      )}
    </>
